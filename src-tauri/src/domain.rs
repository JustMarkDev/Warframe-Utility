    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AppError::Overlap {
                ...
                write!(
                    f,
                    "Item '{}' belongs to multiple eligible factions: {:?}",
                    item_slug, eligible_factions
                )
            }
            AppError::InsufficientStanding { item_slug } => {
                write!(
                    f,
                    "No represented faction has enough standing to sell '{}'",
                    item_slug
                )
            }
...
pub fn get_syndicate_mods_at_rank(faction: &str, rank: i32) -> Vec<&'static str> {
    match (faction, rank) {
        ...
        ("new_loka", 5) => vec!["spellbound_harvest"],
